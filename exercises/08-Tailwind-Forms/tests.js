...
describe("All the tests should pass", function () {
    ...
    it("You should not change or delete the existing elements in the <head> tag", function () {
        ...
        let scriptIndex = head.innerHTML.indexOf("https://cdn.tailwindcss.com");
        let title = head.querySelector("title").innerHTML;

        expect(meta.length).toBe(2);
        expect(scriptIndex > -1).toBeTruthy();
        expect(title).toBe("08 Tailwind Forms");
    });
    ...
    it("The form should have a dark-grey background color", () => {
        let form = document.querySelector("form");
        expect(form).toBeTruthy();
        expect(form.classList.contains("bg-gray-600") || form.classList.contains("bg-gray-700")).toBeTruthy();
    });
    ...
    it("The form should have a button with a blue background color", () => {
        ...
        let button = form.querySelector("button");
        expect(button).toBeTruthy();
        expect(button.classList.contains("bg-blue-600") || button.classList.contains("bg-blue-500")).toBeTruthy();
    });
